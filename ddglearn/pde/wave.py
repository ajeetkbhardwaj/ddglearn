"""Wave equation time-stepping (Crank-Nicolson / Newmark-beta).

Implicit scheme:  (M + dt^2/2 W) u^{n+1} = 2 M u^n - (M + dt^2/2 W) u^{n-1}
...
def wave_step(mesh, u_prev: np.ndarray, u_curr: np.ndarray, dt: float, pin_index: Optional[int] = 0) -> np.ndarray:
    ...
    if _HAS_SCIPY and isspmatrix(D0):
        ...
        W = D0.T @ H1 @ D0
        A = (H0 + 0.5 * dt * dt * W).tocsr()
        rhs = 2.0 * (H0 @ u_curr) - A @ u_prev
