...
class HalfEdgeMesh:
    ...
    def vertex_area_voronoi(self) -> np.ndarray:
        ...
        for idx, f in enumerate(F):
            ...
            vj_vi = vj - vi
            vk_vi = vk - vi
            cross_i = np.cross(vj_vi, vk_vi)
            norm_i = np.linalg.norm(cross_i)
            cot_i = np.dot(vj_vi, vk_vi) / norm_i if norm_i > 1e-12 else 0.0

            vi_vj = vi - vj
            vk_vj = vk - vj
            cross_j = np.cross(vi_vj, vk_vj)
            norm_j = np.linalg.norm(cross_j)
            cot_j = np.dot(vi_vj, vk_vj) / norm_j if norm_j > 1e-12 else 0.0

            vi_vk = vi - vk
            vj_vk = vj - vk
            cross_k = np.cross(vi_vk, vj_vk)
            norm_k = np.linalg.norm(cross_k)
            cot_k = np.dot(vi_vk, vj_vk) / norm_k if norm_k > 1e-12 else 0.0

            e_ij_len_sq = np.sum((vj - vi) ** 2)
            e_jk_len_sq = np.sum((vk - vj) ** 2)
            e_ki_len_sq = np.sum((vi - vk) ** 2)

            if i in boundary_verts:
                A[i] += face_areas[idx] * 0.5
            else:
                A[i] += 0.125 * (cot_k * e_ij_len_sq + cot_j * e_ki_len_sq)

            if j in boundary_verts:
                A[j] += face_areas[idx] * 0.5
            else:
                A[j] += 0.125 * (cot_i * e_jk_len_sq + cot_k * e_ij_len_sq)

            if k in boundary_verts:
                A[k] += face_areas[idx] * 0.5
            else:
                A[k] += 0.125 * (cot_j * e_ki_len_sq + cot_i * e_jk_len_sq)
