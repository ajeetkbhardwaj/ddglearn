...
def cg_solve(
    ...
    Args:
        ...
        active_mask: Boolean tensor of active (non-Dirichlet) vertices
    ...
    # Early exit if initial guess is perfect
    if rz_old < 1e-15:
        return x
...
def implicit_heat_step(
    ...
    if dirichlet_indices is not None and dirichlet_values is not None:
        ...
        x0[pin_index] = 0.0
        active_mask[pin_index] = False

    precond = torch.clamp(mesh.vertex_area_voronoi_tensor, min=1e-12)
    return cg_solve(A, rhs, x0=x0, max_iter=n, tol=1e-8, preconditioner=precond, active_mask=active_mask)
...
def wave_step(
    ...
    # A = H0 is just a diagonal matrix (vertex areas), we can directly divide
    diag = torch.clamp(mesh.vertex_area_voronoi_tensor, min=1e-12)
    u_next = rhs / diag

    if pin_index is not None:
        u_next[pin_index] = 0.0
...
def geodesic_distance(
    ...
    precond = torch.clamp(mesh.vertex_area_voronoi_tensor, min=1e-12)
    u = cg_solve(A, rhs, max_iter=n, tol=1e-8, preconditioner=precond)
...
def hodge_decomposition(
    ...
    from operators.torch_operators import gradient, divergence
    ...
    div_free_part = v - grad_f
    harmonic_part = torch.zeros_like(v)
