...
def hodge_decomposition(mesh, v_edges: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    """
    # Step 1: compute curl-free part via solving ∇² f = div(v)
    div_v = divergence(mesh, v_edges)  # shape (n_vertices,)
    f = solve_poisson(mesh, -div_v, pin_index=0, pin_value=0.0)
    grad_f = gradient(mesh, f)

    # Step 2: divergence-free part (including harmonic) is the residual
    div_free_part = v_edges - grad_f
    
    # Step 3: harmonic part (typically empty for disk topology surfaces)
    harmonic_part = np.zeros_like(v_edges)
